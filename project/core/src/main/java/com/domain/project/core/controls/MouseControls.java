...
//            System.out.println(xOffset + " " + yOffset);
            ...
            //set x bounds
            if(env.getX() >= Const.WORLD_ORIGIN_X && env.getX() <= Const.WORLD_END_X - Const.WINDOW_WIDTH) {
                env.setX(env.getX() - xOffset);
            }
            if(env.getX() < Const.WORLD_ORIGIN_X) {
                env.setX(Const.WORLD_ORIGIN_X);
            }
            if(env.getX() > Const.WORLD_END_X - Const.WINDOW_WIDTH) {
                env.setX(Const.WORLD_END_X - Const.WINDOW_WIDTH);
            }

            //set y bounds
            if(env.getY() >= Const.WORLD_ORIGIN_Y && env.getY() <= Const.WORLD_END_Y - Const.WINDOW_HEIGHT) {
                env.setY(env.getY() - yOffset);
            }
            if(env.getY() < Const.WORLD_ORIGIN_Y) {
                env.setY(Const.WORLD_ORIGIN_Y);
            }
            if(env.getY() > Const.WORLD_END_Y - Const.WINDOW_HEIGHT) {
                env.setY(Const.WORLD_END_Y - Const.WINDOW_HEIGHT);
            }
            xOld = event.x();
            yOld = event.y();
        }
